...
## Dependencies
The script is written using Perl, and was tested on *perl v 5.32.1*. The only side package that is used in decomposition analyses is "Math" to compute CDF. Generally it is pre-installed with Perl itself, but if it is missing, you can use conda to install it 
```
